...
PROBLEMS FOUND IN CURRENT FILE:
 ...
 11. Flight path should use absolute altitude + extrude so it floats at 250ft AGL
     and draws a "curtain" down to the ground (cartoon flight look)

FIXES APPLIED:
  - All <name> tags correct
  - Path: absolute altitude 76.2m (250ft), extrude=1, tessellate=1 → curtain wall
  - Path line: width=5, bright orange ff2b6bff (ABGR) 
  - Waypoints: absolute altitude, extrude=1 → vertical poles to ground
...
GOOGLE EARTH VISUAL (cartoon flight look):
  - Orange extruded path "wall" from 250ft down to ground
...
def diagnose(html: str) -> list[str]:
    ...
    checks = [
        ...
        ("Path has extrude=1",      "extrude>1<" in kml_section or "<extrude>1</extrude>" in kml_section,
                                     "No extrude on flight path — line lies on ground, not at altitude"),
        ("Altitude is absolute",    "absolute" in kml_section,          "relativeToGround not rendered as 3D in older GE versions"),
...
def build_kml(waypoints: list[tuple[float,float]], fov: int, alt_ft: int) -> str:
    ...
    # ── Flight path coords — absolute altitude ───────────────────────────────
    path_coord_str = '\n              '.join(
...
<Document>
  ...
  <!-- Flight path: thick orange extruded wall at {alt_ft}ft AGL -->
  <Style id="pathStyle">
    <LineStyle>
  ...
  <!-- FOLDER 1: Flight path at {alt_ft}ft AGL with extruded wall to ground -->
  <Folder>
...
def validate_kml(kml: str) -> list[str]:
    ...
    for tag in ['<name>', '</name>', '<Document>', '</Document>',
                ...
                '<LookAt>', '<extrude>1</extrude>', 'altitudeMode']:
        if tag not in kml:
...
function exportKMZ() {
  ...
  // ── Waypoint placemarks with extrude poles ────────────────────────────────
...
def main():
    ...
    checks = [
        ...
        ("extrude=1 on flight path",         '<extrude>1</extrude>' in new_html),
        ("absolute altitudeMode",            '<altitudeMode>absolute</altitudeMode>' in new_html),
    ...
    if all_ok:
        ...
        print("     - Orange extruded wall at 250ft AGL (cartoon flight look)")
